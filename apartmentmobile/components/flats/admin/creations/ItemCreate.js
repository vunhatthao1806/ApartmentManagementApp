...
const ItemCreate = () => {
  ...
  const [message, setMessage] = useState(
    "Bạn có bưu kiện mới chưa nhận! Hãy tới quầy nhận bưu kiện nhận hàng nhé! Chân thành cảm ơn!"
  );
  const [phoneNumbers, setPhoneNumbers] = useState([]);
  const [ownerPhoneNumber, setOwnerPhoneNumber] = useState("");

  const StatusTags = tags.filter((t) => t.id === 5);
  ...
  return (
    <ScrollView style={{ backgroundColor: "#FBF6EE" }}>
      ...
      <View style={[Style.margin, Style.container]}>
        <View style={MyStyles.row}>
          <Icon source="check-circle-outline" color={"#4F6F52"} size={28} />
          <Text style={Style.titleComplaint}>Tình trạng hàng:</Text>
        </View>
          ...
          <Picker
            ...
            onValueChange={(itemValue) => {
              ...
              if (selectedCabinet) {
                setOwnerPhoneNumber(
                  phoneNumbers.find(
                    (pn) => pn.id === selectedCabinet.phone_number
                  )?.number || ""
                );
              }
            }}
