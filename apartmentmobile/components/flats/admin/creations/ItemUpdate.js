...
const ItemUpdate = () => {
  ...
  const toggleItemStatus = async (itemId) => {
    try {
      ...
      const newStatusId =
        updatedItem.status_tag.name === "Chưa nhận hàng" ? 5 : 6; // Sử dụng ID đúng của trạng thái
      let accessToken = await AsyncStorage.getItem("access-token");
  ...
  useEffect(() => {
    if (selectedEcabinet) {
      const selectedCabinetItems = items.filter(
        (i) => i.e_cabinet === selectedEcabinet
      );
        ...
        {ecabinets === null ? (
          ...
          <Picker
            ...
            onValueChange={(itemValue) => {
              ...
              if (selectedCabinet) {
                setSelectedItems(
                  items.filter((i) => i.e_cabinet === selectedCabinet.id)
                );
              }
            }}
          >
              ...
              selectedItems.map((item) => (
                <View key={item.id} style={{ width: "50%", marginBottom: 8 }}>
                  <Card style={{ flex: 1, marginLeft: 7 }}>
                    ...
                    <Text style={{ alignSelf: "center", marginBottom: 4 }}>
